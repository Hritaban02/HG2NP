...
class Sampler(nn.Module):
    def __init__(self, cfg, types):
        ...
        i = 0
        n = len(cfg["hidden_layers"])
        # Ablation Homogeneous MPNN
        self.Linear_Layers.add_module(f"Linear_{i}", nn.Linear(in_features=cfg["num_categories"], out_features=cfg["hidden_layers"][0]))
        i+=1
...
class ablation_hom_HETModel(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        ...
        # Ablation Homogeneous MPNN
        if self.dataset_name=="DBLP":
            cfg["sampler"]["num_categories"]=4
        if self.dataset_name=="IMDB":
            cfg["sampler"]["num_categories"]=3
