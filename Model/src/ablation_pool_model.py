...
class ablation_pool_HETModel(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        ...
        for key in self.category_map:
            ...
            self.features.update({self.category_map[key]:Sampler(cfg=cfg["sampler"], types=self.sample_pools[key].shape[1])})
        ...
        with ClearCache():
            ...
            for data in batch.to_data_list():
                ...
                for key in self.category_map:
                    ...
                    # Ablation Node Pool
                    d[self.category_map[key]]['x']=self.features[self.category_map[key]](graph_x[indexToPosition[key]])
