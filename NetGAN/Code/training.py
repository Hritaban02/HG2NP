...
class Trainer():
    def __init__(self, graph,  N, max_iterations=20000, rw_len=16, batch_size=128, H_gen=40, H_disc=30, H_inp=128, z_dim=16, lr=0.0003, n_critic=3, gp_weight=10.0, betas=(.5, .9),
        ...
        #self.walker = stuff[6]
        self.train_ones, self.val_ones, self.val_zeros, self.test_ones, self.test_zeros = Code.utils.train_val_test_split_adjacency(graph, val_share, test_share, seed, undirected=True, connected=True, asserts=True, set_ops=set_ops)
        ...
        self.walker = Code.utils.RandomWalker(self.train_graph, rw_len, p=1, q=1, batch_size=batch_size)
        self.eo = []
        self.critic_loss = []
        self.generator_loss = []
        self.avp = []
    ...
    def create_graph(self, num_samples, i, batch_size=1000, reset_weights=False):
        ...
        print("Number iterations: " + str(num_iterations), flush=True)
        for j in range(num_iterations):
            if(j%10 == 1): print(j, flush=True)
            samples.append(self.generator.sample_discrete(batch_size, self.device))
        samples = np.vstack(samples)
        gr = Code.utils.score_matrix_from_random_walks(samples, self.N)
        gr = gr.tocsr()

        # Assemble a graph from the score matrix
        _graph = Code.utils.graph_from_scores(gr, self.graph.sum())
        # Compute edge overlap
        edge_overlap = Code.utils.edge_overlap(self.graph.toarray(), _graph)
        ...
        print('roc: {:.4f}   avp: {:.4f}   eo: {:.4f}'.format(self.roc_auc[-1], self.avp[-1], self.eo[-1]), flush=True)
    ...
    def check_running(self, i):
        if (self.stopping_criterion == 'val'):
            if (self.roc_auc[-1] + self.avp[-1] > self.best_performance):
                ...
                self.patience -= 1
            if self.patience == 0:
                print('finished after {} iterations'.format(i), flush=True)
                self.running = False
        else:
    ...
    def initialize_validation_settings(self, stopping_criterion, stopping_eo, max_patience):
        ...
        if (self.stopping_criterion == 'val'):
            print("**** Using VAL criterion for early stopping with max patience of: {}****".format(self.max_patience), flush=True)
        else:
            assert self.stopping_eo is not None, "stopping_eo is not a float"
            print("**** Using EO criterion of {} for early stopping".format(self.stopping_eo), flush=True)

    def plot_graph(self, i=None, folder=None):
        ...
        if (i is not None) and (folder is not None):
            os.makedirs(folder, exist_ok=True)
            plt.savefig(f"{folder}/{i}.png")

        plt.show()

    def train(self, create_graph_every = 2000, plot_graph_every=500, num_samples_graph = 100000, stopping_criterion='val', max_patience=5, stopping_eo=None, folder=None):
        """
        ...
        for i in range(self.max_iterations):
            if(self.running):
                ...
                if(i%10 ==1): print('iteration: {}      critic: {:.6f}      gen {:.6f}'.format(i, self.critic_loss[-1], self.generator_loss[-1]), flush=True)
                if (i % create_graph_every == create_graph_every-1):
                    self.create_graph(num_samples_graph,  i)
                    self.check_running(i)
                    print('Took {} minutes so far..'.format((time.time() - starting_time)/60), flush=True)
                if plot_graph_every > 0 and (i + 1) % plot_graph_every == 0: self.plot_graph(i, folder)