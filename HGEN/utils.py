    ...
    if every_node:
        if connected:
            ...
            if use_edge_cover:
                ...
                hold_edges_d1 = np.column_stack((not_in_cover[d_nic > 0], np.row_stack(list(map(np.random.choice, A[not_in_cover[d_nic > 0]].tolil().rows)))))

                if np.any(d_nic == 0):
                    hold_edges_d0 = np.column_stack((np.row_stack(list(map(np.random.choice, A[:, not_in_cover[d_nic == 0]].T.tolil().rows))), not_in_cover[d_nic == 0]))
